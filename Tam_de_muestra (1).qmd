...
modeloExp <- RMtrend(10) + RMexp(var = 1, scale = rango_practico/3)
# En el caso gaussiano el rango práctico es scale=1/sqrt(3)*(rango_práctico)
modeloGauss <- RMtrend(10) + RMgauss(var=1,scale = rango_practico/sqrt(3))
...
modeloMat <- RMtrend(10) + RMmatern(var = 1, nu = kappa, scale = rango_practico / r1)
```
...
varioemp <- RFvariogram(data = simuMat)
plot(varioemp, model = modeloMat, ylim = c(0, 1))
```
...
```{r}
set.seed(NULL)
mc1 <- variog.mc.env(datos1g, 
                     obj = semivariog1, 
                     nsim = 95)
plot(semivariog1, env = mc1,
     xlab = "Distancia", 
     ylab = "Semivarianza", 
     pch = 19, 
     lwd = 2)
...
simuGauss2  <- RFsimulate(modeloGauss2,  
                          x = sec.x, y = sec.y, grid=TRUE)
...
for (i in 1:nsimul){
  ...
  semivariog1  <- variog(datosGaussGD, uvec = seq(25, 750, by =25),
                        messages = FALSE)
  mc1 <- variog.mc.env(datosGaussGD, obj = semivariog1, nsim = 95,
                       messages = FALSE)
